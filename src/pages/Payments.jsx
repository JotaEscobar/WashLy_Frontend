...
import { ModalDiario } from '../components/payments/ModalDiario';
import { ModalDetail } from '../components/payments/ModalDetail';
import { ModalMovimiento } from '../components/payments/ModalMovimiento';
const Payments = () => {
    // --- ESTADOS ---
    ...
    useEffect(() => {
        if (currentSede) fetchCaja();
    }, [currentSede?.id]);

    useEffect(() => {
        if (currentSede) fetchTableData();
    }, [searchTerm, statusFilter, fechaDesde, fechaHasta, currentSede?.id]);
    ...
    const handleRegisterMovimiento = async () => {
        ...
        try {
            await api.post(`pagos/caja/${caja.id}/movimiento/`, {
                ...
                metodo_pago_id: movimientoForm.metodo_pago,
        ...
        modalConfig.show && (
            <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-sm shadow-2xl text-center border border-gray-100 dark:border-gray-700">
                    <div className={`mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-4 ${modalConfig.type === 'error' ? 'bg-red-100 text-red-500' :
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            ...
                            className={`flex-1 text-white py-2.5 rounded-xl font-bold transition-colors ${modalConfig.type === 'error' ? 'bg-red-600 hover:bg-red-700' :
    ...
    if (!caja) {
        return (
            <div className="h-full flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
                ...
                {renderGlobalModal()}
                <ModalDiario showDiarioModal={showDiarioModal} setShowDiarioModal={setShowDiarioModal} diarioFilters={diarioFilters} setDiarioFilters={setDiarioFilters} fetchDiario={fetchDiario} diarioEvents={diarioEvents} loadingDiario={loadingDiario} openTransactionDetail={openTransactionDetail} today={today} />
                <ModalDetail showDetailModal={showDetailModal} setShowDetailModal={setShowDetailModal} detailContent={detailContent} />
            </div>
        );
    }
    ...
    return (
        <div className="p-6 h-full flex flex-col text-gray-800 dark:text-gray-100 relative">
            ...
            {/* MODALES GLOBALES */}
                <ModalDiario showDiarioModal={showDiarioModal} setShowDiarioModal={setShowDiarioModal} diarioFilters={diarioFilters} setDiarioFilters={setDiarioFilters} fetchDiario={fetchDiario} diarioEvents={diarioEvents} loadingDiario={loadingDiario} openTransactionDetail={openTransactionDetail} today={today} />
                <ModalDetail showDetailModal={showDetailModal} setShowDetailModal={setShowDetailModal} detailContent={detailContent} />
                <ModalMovimiento showMovimientoModal={showMovimientoModal} setShowMovimientoModal={setShowMovimientoModal} movimientoForm={movimientoForm} setMovimientoForm={setMovimientoForm} handleRegisterMovimiento={handleRegisterMovimiento} />
        </div>
    );
};
